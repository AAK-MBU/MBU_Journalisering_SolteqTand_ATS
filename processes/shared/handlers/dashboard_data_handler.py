...
def handle_process_dashboard(
    ...
    try:
        ...
        step_run_id = process_step_run.get_step_run_id_for_process_step_cpr(
            client=client,
            process_name=get_context_values("process_name"),
...
def update_process_run_metadata(item_data: dict, process_name: str) -> dict:
    ...
    try:
        ...
        logger.info(
            "Process run metadata updated successfully: %s", process_run_metadata
        )

        return response.json()
    except Exception as e:
        logger.error("Error updating process run metadata: %s", e)
        raise e
